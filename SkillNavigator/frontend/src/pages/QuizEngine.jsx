  ...
  while (currentLevel <= 5) {
    ...
    if (nextTag) {
      ...
      if (nextLevelQuestions.length > 0) {
        // Group questions by pathId to avoid asking duplicate questions across different paths in the same stream
        const pathsMap = {};
        nextLevelQuestions.forEach(q => {
          const id = q.id || q.Id || "";
          if (id.startsWith("hq-g-")) {
            const parts = id.split("-");
            const pathId = parts.slice(2, parts.length - 1).join("-");
            if (!pathsMap[pathId]) pathsMap[pathId] = [];
            pathsMap[pathId].push(q);
          }
        });

        const uniquePaths = Object.keys(pathsMap);
        if (uniquePaths.length > 0) {
          // Select the first representative path for this domain level
          const selectedPathId = uniquePaths[0];
          const pathQuestions = pathsMap[selectedPathId];

          // Select one Interest (i1), one WorkStyle (w1), and one Aptitude (a1) question to keep it concise and distinct
          const selectedQuestions = [];
          const interestQ = pathQuestions.find(q => (q.id || q.Id || "").endsWith("-i1"));
          if (interestQ) selectedQuestions.push(interestQ);
          const workStyleQ = pathQuestions.find(q => (q.id || q.Id || "").endsWith("-w1"));
          if (workStyleQ) selectedQuestions.push(workStyleQ);
          const aptitudeQ = pathQuestions.find(q => (q.id || q.Id || "").endsWith("-a1"));
          if (aptitudeQ) selectedQuestions.push(aptitudeQ);

          if (selectedQuestions.length === 0) {
            selectedQuestions.push(...pathQuestions.slice(0, 3));
          }
          path.push(...selectedQuestions);
        } else {
          path.push(...nextLevelQuestions.slice(0, 3));
        }
      } else {
...
export default function QuizEngine() {
  ...
  const tooltipStyle = isDark 
    ? { backgroundColor: '#0f172a', borderColor: '#1e293b', borderRadius: '12px', color: '#f8fafc', fontSize: 11 }
    : { backgroundColor: '#ffffff', borderColor: '#e2e8f0', borderRadius: '12px', color: '#0f172a', fontSize: 11 };

  return (
    <div className="min-h-[calc(100vh-80px)] w-full bg-background text-foreground flex flex-col items-center justify-center p-4 relative overflow-y-auto">
      {/* Background gradients */}
      <div className="absolute top-10 left-10 w-96 h-96 bg-indigo-500/5 dark:bg-indigo-500/10 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-10 right-10 w-96 h-96 bg-cyan-500/5 dark:bg-cyan-500/10 rounded-full blur-[100px] pointer-events-none" />
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'landing' && (
          ...
          >
            <div className="space-y-3">
              <span className="text-xs font-semibold bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/25 px-3 py-1 rounded-full uppercase tracking-wider">
                Interactive Career Discovery Ecosystem
              </span>
              <h1 className="text-3xl md:text-5xl font-black bg-gradient-to-r from-indigo-600 via-purple-600 to-cyan-600 dark:from-white dark:via-indigo-200 dark:to-cyan-300 bg-clip-text text-transparent leading-tight pt-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-8 max-w-4xl mx-auto">
              {Object.entries(STAGE_METADATA).map(([key, meta]) => (
                <button
                  ...
                  className="group relative flex flex-col items-center p-6 rounded-2xl bg-card border border-border hover:border-indigo-500/40 transition-all duration-300 hover:scale-102 hover:shadow-lg hover:shadow-indigo-500/5 text-center overflow-hidden h-full"
                >
                  ...
                  <div className={`p-3 rounded-xl bg-background border border-border text-indigo-600 dark:text-indigo-400 mb-4 group-hover:scale-110 transition-transform duration-300`}>
                    {meta.icon}
                  </div>
                  <h3 className="font-bold text-foreground text-sm mb-1">{meta.title}</h3>
                  <p className="text-[11px] text-muted-foreground group-hover:text-foreground transition-colors leading-normal flex-1">
                    {meta.desc}
                  </p>
                  <ChevronRight className="w-4 h-4 text-muted-foreground group-hover:text-foreground mt-4 transition-transform duration-300 group-hover:translate-x-1" />
                </button>
              ))}
            </div>
          </motion.div>
        ...
        {step === 'quiz' && activeQuestions.length > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="w-full max-w-2xl bg-card border border-border/80 backdrop-blur-md rounded-2xl p-6 md:p-8 space-y-6 z-10 shadow-xl"
          >
            {/* Header / Progress */}
            <div className="flex items-center justify-between border-b border-border/60 pb-4">
              ...
              <div className="flex items-center gap-1 bg-background border border-border px-3 py-1 rounded-full text-[10px] text-muted-foreground">
                <Compass className="w-3.5 h-3.5 text-cyan-500 animate-spin-slow animate-spin" />
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-xl border text-sm font-semibold transition-all duration-300 flex items-center justify-between gap-4 group ${
                      isSelected
                        ? 'bg-indigo-500/10 dark:bg-indigo-950/30 border-indigo-500 text-foreground shadow-md'
                        : 'bg-secondary/40 border-border hover:border-indigo-500/40 text-muted-foreground hover:text-foreground'
                    }`}
                  >
                    ...
                    <span className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-colors ${
                      isSelected ? 'border-indigo-400 bg-indigo-500 text-white' : 'border-border group-hover:border-indigo-500/40'
                    }`}>
            ...
            <div className="flex justify-between items-center pt-2">
              ...
              <span className="text-[10px] text-muted-foreground">Adaptive Decision Tree System</span>
            </div>
          </motion.div>
        )}
        ...
        {step === 'loading' && (
          ...
          >
            ...
            <div className="space-y-1">
              <h3 className="font-bold text-lg text-foreground">Analyzing Your Inputs</h3>
        ...
        {step === 'result' && resultProfile && (
          ...
          >
            {/* Header Result Card */}
            <div className="p-6 md:p-8 rounded-2xl bg-gradient-to-b from-indigo-500/5 to-card border border-indigo-500/30 dark:from-indigo-950/40 dark:to-slate-900/40 backdrop-blur-md shadow-2xl relative overflow-hidden text-card-foreground">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                <div className="space-y-2">
                  <span className="text-xs font-bold uppercase bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 border border-indigo-500/30 px-3 py-1 rounded-full tracking-wider">
                ...
                <div className="flex items-center gap-4 shrink-0 bg-background/80 border border-border/80 p-4 rounded-xl">
                  <div className="text-center">
                    <p className="text-[10px] text-muted-foreground uppercase font-bold">Confidence</p>
                    <p className="text-2xl font-black text-indigo-600 dark:text-indigo-400">{resultProfile.confidenceScore?.toFixed(1)}%</p>
                  </div>
                  <div className="h-8 w-px bg-border" />
                  <div className="text-center">
                    <p className="text-[10px] text-muted-foreground uppercase font-bold">Est. Salary</p>
              ...
              <div className="mt-6 pt-6 border-t border-border/80 flex flex-wrap gap-3 items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <span className="text-xs text-muted-foreground">Target Skills Needed:</span>
                  <div className="flex flex-wrap gap-1.5">
                    {resultProfile.skillGap?.slice(0, 3).map((s, idx) => (
                      <span key={idx} className="text-[11px] bg-background text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 px-2.5 py-0.5 rounded-full font-medium">
                        {s}
                      </span>
                    ))}
                    {resultProfile.skillGap?.length > 3 && (
                      <span className="text-[10px] text-muted-foreground font-semibold mt-0.5">+{resultProfile.skillGap.length - 3} more</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <Button
                    ...
                    variant="outline"
                    className="border-border bg-background/80 hover:bg-secondary text-xs gap-1.5 text-foreground h-8"
                  >
                    <Bookmark className={`w-4 h-4 ${bookmarked ? 'fill-indigo-500 text-indigo-500 dark:fill-indigo-400 dark:text-indigo-400' : 'text-muted-foreground'}`} />
                    {bookmarked ? 'Saved' : 'Bookmark'}
                  </Button>
                  <Button
                    onClick={() => handleAcceptCareer(resultProfile.recommendedCareer?.id)}
                    disabled={accepting}
                    className="text-xs h-8 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white border-0 font-bold"
                  >
                    {accepting ? (
                      <>
                  ...
                  >
                    <Link to={`/career-explorer?highlight=${resultProfile.recommendedCareer?.id}`}>
                      View Career Map
                    </Link>
                  </Button>
                  <Button
                    onClick={handleReset}
                    variant="ghost"
                    className="text-muted-foreground hover:text-foreground hover:bg-secondary/60 text-xs h-8"
                  >
            ...
            {/* Tabs Header */}
            <div className="flex gap-6 bg-secondary p-1.5 rounded-xl border border-border">
              <button 
                ...
                className={`px-4 py-2 text-xs font-bold transition-all rounded-lg flex items-center gap-1.5 ${
                  activeResultTab === 'overview' 
                    ? 'bg-card text-indigo-600 dark:text-indigo-400 border border-border shadow-sm' 
            ...
            <div className="min-h-[300px]">
                ...
                <div className="space-y-4 animate-in fade-in duration-300">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
                    <div className="p-4 rounded-xl bg-card border border-border">
                      <span className="text-[10px] text-muted-foreground font-semibold block mb-1">Growth rate CAGR</span>
                      <span className="text-sm font-bold text-orange-500 dark:text-orange-400">{resultProfile.recommendedCareer?.industryGrowth}%</span>
                    </div>
                    <div className="p-4 rounded-xl bg-card border border-border">
                      <span className="text-[10px] text-muted-foreground font-semibold block mb-1">Automation Risk</span>
                      <span className={`text-sm font-bold ${resultProfile.recommendedCareer?.automationRisk >= 50.0 ? 'text-rose-500 dark:text-rose-400' : 'text-emerald-500 dark:text-emerald-400'}`}>
                        {resultProfile.recommendedCareer?.automationRisk}%
                      </span>
                    </div>
                    <div className="p-4 rounded-xl bg-card border border-border">
                      <span className="text-[10px] text-muted-foreground font-semibold block mb-1">Remote Capability</span>
                      <span className="text-sm font-bold text-foreground">{resultProfile.recommendedCareer?.remoteWorkPossibility}%</span>
                    </div>
                    <div className="p-4 rounded-xl bg-card border border-border">
                      <span className="text-[10px] text-muted-foreground font-semibold block mb-1">Difficulty level</span>
                      <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400">{resultProfile.recommendedCareer?.difficultyLevel || 'Medium'}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeResultTab === 'matches' && (
                <div className="space-y-3 animate-in fade-in duration-300">
                  <p className="text-xs text-muted-foreground mb-2">Here are the top 10 career pathways matching your responses in the {STAGE_METADATA[selectedStream]?.title} category:</p>
                  {(resultProfile.topMatches || []).map((match, idx) => {
                    ...
                    return (
                      <div key={match.id} className="border border-border rounded-xl bg-card/60 overflow-hidden">
                        {/* Summary Header */}
                        <div 
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <span className="w-6 h-6 rounded-full bg-background border border-border text-xs font-bold text-indigo-600 dark:text-indigo-400 flex items-center justify-center">
                              {idx + 1}
                            </span>
                            <div>
                              <h4 className="text-xs font-bold text-foreground">{match.title}</h4>
                              <p className="text-[10px] text-muted-foreground">Level {match.level} • CAGR: {match.industryGrowth}%</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <span className="text-[10px] font-mono text-indigo-600 dark:text-indigo-400 font-bold shrink-0">{matchScore}% Match</span>
                        ...
                        {/* Collapsible Content */}
                        {isExpanded && (
                          <div className="p-4 bg-background/40 border-t border-border/60 space-y-3 text-xs">
                            <p className="text-muted-foreground leading-normal">{match.description}</p>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-2 text-[10px]">
                              <div>
                                <span className="text-muted-foreground block">Required Degrees:</span>
                                <span className="font-semibold text-foreground">{match.requiredDegrees?.join(', ') || 'N/A'}</span>
                              </div>
                              <div>
                                <span className="text-muted-foreground block">Est. Max Salary:</span>
                                <span className="font-semibold text-emerald-600 dark:text-emerald-400">₹{(maxSal/100000).toFixed(1)}L/yr</span>
                              </div>
                              <div>
                                <span className="text-muted-foreground block">AI Displ. Risk:</span>
                                <span className={`font-semibold ${match.automationRisk >= 50.0 ? 'text-rose-500 dark:text-rose-400' : 'text-emerald-500 dark:text-emerald-400'}`}>
                                  {match.automationRisk}%
                                </span>
                              </div>
                              <div>
                                <span className="text-muted-foreground block">Environment:</span>
                                <span className="font-semibold text-foreground">{match.workEnvironment}</span>
                              </div>
                            </div>
                            <div className="pt-3 flex gap-2 justify-end">
                              <Button asChild size="sm" variant="outline" className="h-7 text-[10px] border-border bg-background">
                                <Link to={`/career-insights?id=${match.id}`}>Explore Insights</Link>
                              </Button>
                              <Button asChild size="sm" className="h-7 text-[10px] bg-indigo-600 hover:bg-indigo-500 text-white border-0">
              ...
              {activeResultTab === 'strengths' && (
                <div className="space-y-5 animate-in fade-in duration-300">
                        ...
                        <ul className="space-y-2">
                          {strengths.map((str, idx) => (
                            <li key={idx} className="text-xs text-foreground flex items-start gap-2">
                              <ChevronRight className="w-3.5 h-3.5 text-emerald-500 dark:text-emerald-400 mt-0.5 shrink-0" />
                              <span>{str}</span>
                            </li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-xs text-muted-foreground italic">No specific strengths mapped. Retake the quiz for higher fidelity scores.</p>
                      )}
                    </div>
                      ...
                      {weaknesses.length > 0 ? (
                        <ul className="space-y-2">
                          {weaknesses.map((wk, idx) => (
                            <li key={idx} className="text-xs text-foreground flex items-start gap-2">
                              <ChevronRight className="w-3.5 h-3.5 text-rose-500 dark:text-rose-400 mt-0.5 shrink-0" />
                              <span>{wk}</span>
                            </li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-xs text-muted-foreground italic">No major vulnerabilities detected in this stream category.</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-5 rounded-xl border border-border bg-card/60">
                    <h3 className="font-bold text-xs text-foreground mb-4 flex items-center gap-2">
                      <BookOpen className="w-4 h-4 text-indigo-600 dark:text-indigo-400" /> Sequential Learning Milestones
                    </h3>
                    <div className="space-y-4 relative before:absolute before:top-2 before:bottom-2 before:left-[11px] before:w-[2px] before:bg-border">
                      {(resultProfile.roadmap || []).map((item, idx) => (
                        <div key={idx} className="flex gap-4 relative z-10">
                          <span className="w-6 h-6 rounded-full bg-background border border-border text-[11px] font-bold text-indigo-600 dark:text-indigo-400 flex items-center justify-center shrink-0 shadow-lg">
                            {idx + 1}
                          </span>
                          <div>
                            <h4 className="text-xs font-bold text-foreground">{item.phase || item.Phase}</h4>
                            <p className="text-[10px] text-muted-foreground mt-0.5">Duration: {item.duration || item.Duration}</p>
                            {item.topics && (
                              <div className="flex flex-wrap gap-1 mt-1.5">
                                {(item.topics || []).map((top, tIdx) => (
                                  <span key={tIdx} className="text-[9px] bg-background px-2 py-0.5 rounded text-muted-foreground border border-border/40">
                                    {top}
                                  </span>
                                ))}
                              </div>
                ...
                <div className="space-y-5 animate-in fade-in duration-300">
                  <div className="p-5 rounded-xl border border-border bg-card/60">
                    <div className="flex justify-between items-center mb-4">
                      <div>
                        <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">10-Year Salary Trajectory Projection</h4>
                        <p className="text-[10px] text-muted-foreground">Based on industry growth rate of {resultProfile.recommendedCareer?.industryGrowth}% CAGR</p>
                      </div>
                      <span className="text-xs bg-background border border-border text-emerald-600 dark:text-emerald-400 font-semibold px-2 py-0.5 rounded">
                       ...
                       <div className="h-60 w-full">
                         <ResponsiveContainer width="100%" height="100%">
                           <AreaChart data={salaryChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                             ...
                             <XAxis dataKey="year" stroke={labelColor} fontSize={10} />
                             <YAxis stroke={labelColor} fontSize={10} unit="L" />
                             <Tooltip 
                  ...
                  {/* ROI analysis block */}
                  <div className="p-5 rounded-xl border border-border bg-card/60 flex flex-col md:flex-row justify-between gap-4 items-center">
                    <div className="space-y-1 text-center md:text-left">
                      <h4 className="text-xs font-bold text-foreground flex items-center gap-1.5 justify-center md:justify-start">
                        <Wallet className="w-4 h-4 text-emerald-500" /> ROI and Wealth Generation Rating
                      </h4>
            ...
            <div className="flex gap-2 flex-col sm:flex-row">
              <Button
                onClick={() => handleAcceptCareer(resultProfile.recommendedCareer?.id)}
                disabled={accepting}
                className="flex-1 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold border-0 text-xs animate-none"
              >
                {accepting ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 animate-spin mr-1.5" />
              ...
              <Button asChild variant="outline" className="border-border bg-background/80 hover:bg-secondary text-xs text-muted-foreground">
