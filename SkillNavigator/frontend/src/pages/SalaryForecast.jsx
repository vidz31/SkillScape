...
export default function SalaryForecast() {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-slate-950 text-white p-4 md:p-8 relative overflow-y-auto">
      ...
      <div className="absolute top-[-5%] left-[10%] w-[500px] h-[500px] bg-indigo-500/10 rounded-full blur-[130px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[10%] w-[400px] h-[400px] bg-cyan-500/10 rounded-full blur-[110px] pointer-events-none" />
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-black text-white flex items-center gap-2">
              <TrendingUp className="w-6 h-6 text-indigo-400" /> Salary Forecast Engine
            </h1>
            <p className="text-xs text-slate-400 mt-0.5">Model expected salary cagr trends based on skills, geography, and graduation timelines</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 backdrop-blur-md rounded-2xl p-6 space-y-6">
            <h3 className="font-bold text-sm text-white flex items-center gap-2 border-b border-slate-800/60 pb-3">
              <BrainCircuit className="w-4.5 h-4.5 text-indigo-400" /> Forecast Parameters
            </h3>

            {/* Career Selector */}
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Select Career Role</label>
              {isPathsLoading ? (
                <div className="h-10 bg-slate-950/60 border border-slate-800 rounded-xl flex items-center justify-center">
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm font-semibold text-white focus:outline-none focus:border-indigo-500/50"
                >
                  {careerPaths.map((p) => (
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Target Year</label>
                <span className="text-xs font-bold text-indigo-400">{targetYear}</span>
              </div>
              <input
                type="range"
                min="2026"
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Expected Graduation</label>
                <span className="text-xs font-bold text-cyan-400">{graduationYear}</span>
              </div>
              <input
                type="range"
                min="2026"
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1">
                <MapPin className="w-3.5 h-3.5 text-rose-400" /> Geographic Zone
              </label>
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1">
                <Award className="w-3.5 h-3.5 text-amber-400" /> Target Skill Tier
              </label>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Overlay Loader */}
            {isLoading && !forecast && (
              <div className="h-[400px] bg-slate-900/40 border border-slate-800 rounded-2xl flex items-center justify-center">
            ...
            {forecast && (
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  {/* Predicted Salary */}
                  <div className="bg-gradient-to-b from-indigo-950/30 to-slate-900/30 border border-indigo-500/25 p-5 rounded-2xl shadow-lg">
                    <span className="text-[10px] uppercase font-bold text-indigo-400">Predicted Salary ({forecast.targetYear})</span>
                    <p className="text-2xl font-black text-white mt-2 leading-tight">
                      {formatINR(forecast.predictedSalary)}
                    </p>
                    <p className="text-[10px] text-slate-500 mt-1">Expected starting base salary</p>
                  </div>

                  {/* Salary Range */}
                  <div className="bg-gradient-to-b from-cyan-950/30 to-slate-900/30 border border-cyan-500/25 p-5 rounded-2xl shadow-lg">
                    <span className="text-[10px] uppercase font-bold text-cyan-400">Confidence Range</span>
                    <p className="text-lg font-black text-white mt-2.5 leading-tight">
                      {formatINR(forecast.predictedSalaryMin)} - {formatINR(forecast.predictedSalaryMax)}
                    </p>
                    <p className="text-[10px] text-slate-500 mt-1.5">Based on volatility trends</p>
                  </div>

                  {/* India vs Global Avg */}
                  <div className="bg-gradient-to-b from-emerald-950/30 to-slate-900/30 border border-emerald-500/25 p-5 rounded-2xl shadow-lg">
                    <span className="text-[10px] uppercase font-bold text-emerald-400">Geographic Index</span>
                    <div className="mt-2 text-xs space-y-1">
                      <div className="flex justify-between text-slate-300">
                        <span>India Avg:</span>
                        <span className="font-bold text-white">{formatINR(forecast.indiaSalaryAvg)}</span>
                      </div>
                      <div className="flex justify-between text-slate-300">
                        <span>Global Avg:</span>
                        <span className="font-bold text-white">{formatINR(forecast.globalSalaryAvg)}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-slate-900/30 border border-slate-800/80 rounded-2xl p-6 backdrop-blur-md">
                  <div className="flex justify-between items-center mb-6">
                    <div>
                      <h4 className="font-bold text-sm text-white">Compound Salary Trajectory</h4>
                      <p className="text-[10px] text-slate-500 mt-0.5">Estimated CAGR comparison over the years</p>
                    </div>
                    <span className="text-[10px] font-semibold bg-slate-950 text-indigo-400 border border-indigo-500/25 px-2.5 py-1 rounded-full uppercase tracking-wider">
                ...
                <div className="bg-slate-900/30 border border-slate-800/80 rounded-2xl p-5 flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    <h5 className="font-bold text-xs text-white">Geographic & Skill Multipliers</h5>
